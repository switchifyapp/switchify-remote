...
5. Exercise eight-way movement, all clicks, both scroll directions, drag cleanup, speed limits, and monitor movement. Start movement and scroll repeats, then stop each with Stop movement, another Remote control, and an Android physical switch. Repeat with a held navigation key: confirm it repeats at the desktop's configured cadence after the initial delay, that the control is labelled Stop repeating, and that the same three stops end it. Confirm a key that cannot repeat, such as Escape, still reaches the PC when it stops a repeat. Repeat after backgrounding, reconnecting, and replacing the active Remote session. Confirm the PC stops before the Android switch-capture state clears.
