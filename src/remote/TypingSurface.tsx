...
export function TypingSurface({ session, mode, draft }: { session: RemoteSession; mode: TypingMode; draft: string }) {
  ...
  return <View style={{ gap: spacing.md }}>
    ...
    {mode === 'live' ? <View style={{ gap: spacing.sm }}><StatusBadge icon={liveFailed ? 'error-outline' : 'check-circle'} label={!liveSupported ? 'Live typing is not supported by this PC.' : liveFailed ? 'Some text has not reached your PC.' : 'Live · sent as you type'} tone={liveFailed ? 'danger' : 'success'} />{liveFailed ? <ControlButton label="Retry unsent text" onPress={() => reconcileLive(liveText)} /> : null}</View> : <ResponsiveGrid maxColumns={2} minItemWidth={130}><ControlButton label="Clear" disabled={!draft} onPress={() => void preferencesStore.update({ draft: '' })} /><ControlButton label="Send to PC" disabled={!draft || !draftSupported} onPress={() => void sendDraft()} /></ResponsiveGrid>}
    <AppText accessibilityRole="header" variant="heading">PC keys</AppText>
    <ResponsiveGrid maxColumns={4} minItemWidth={80}>{keys.map((key) => <ControlButton key={key} size="key" label={key.replace('Arrow', '')} disabled={!keySupported} onPress={() => { if (mode === 'live') void session.streamKey(key); else { const [type, payload] = commandPayloads.key(key); void session.command(type, payload); } }} />)}</ResponsiveGrid>
  </View>;
}
