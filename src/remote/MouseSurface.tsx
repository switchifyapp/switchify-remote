...
export function MouseSurface({ session, state, physicalSwitchStopAvailable = true }: { session: RemoteSession; state: RemoteSessionState; physicalSwitchStopAvailable?: boolean }) {
  ...
  const { isExpanded, isLandscape, isMedium } = useLayout();
  const { spacing } = useTheme();
  const twoPane = isExpanded || (isMedium && isLandscape);
  ...
  const movement = <View style={{ flex: 1, gap: spacing.md, maxWidth: 400, minWidth: twoPane ? 320 : undefined, width: '100%' }}>
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 10 }}>{directions.map(([label, accessibleLabel, dx, dy]) => <View key={label} style={{ flexBasis: '30%', flexGrow: 1 }}><ControlButton emphasized={dx === 0 && dy === 0} label={label} accessibilityLabel={accessibleLabel} size="key" disabled={!session.supports(dx === 0 && dy === 0 ? 'mouse.click' : 'mouse.move')} onPress={() => dx === 0 && dy === 0 ? sendTuple(click) : void session.mouse('mouse.move', { dx: dx * step, dy: dy * step }, true)} /></View>)}</View>
  ...
  const secondary = <View style={{ flex: 1, gap: spacing.md, minWidth: twoPane ? 300 : undefined }}>
    ...
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}><ControlButton icon="ads-click" label="Double click" disabled={!session.supports('mouse.doubleClick')} onPress={() => sendTuple(commandPayloads.doubleClick())} /><ControlButton icon="mouse" label="Right click" disabled={!session.supports('mouse.rightClick')} onPress={() => sendTuple(commandPayloads.rightClick())} /><ControlButton icon="pan-tool" label={state.dragging ? 'End drag' : 'Start drag'} disabled={!session.supports(state.dragging ? 'mouse.dragEnd' : 'mouse.dragStart')} selected={state.dragging} onPress={() => void session.toggleDrag()} /></View>
    <View style={{ flexDirection: 'row', gap: spacing.sm }}><ControlButton icon="arrow-upward" label="Scroll up" disabled={!session.supports('mouse.scroll')} onPress={() => void session.mouse('mouse.scroll', { dx: 0, dy: scrollStep }, true)} /><ControlButton icon="arrow-downward" label="Scroll down" disabled={!session.supports('mouse.scroll')} onPress={() => void session.mouse('mouse.scroll', { dx: 0, dy: -scrollStep }, true)} /></View>
    {speed?.supported ? <View style={{ gap: spacing.sm }}><AppText accessibilityRole="header" variant="heading">Pointer speed · {speed.scalePercent}%</AppText><View style={{ flexDirection: 'row', gap: spacing.sm }}><ControlButton icon="remove" label="Slower" disabled={!session.supports('pointer.speed.set') || !speed.setSupported || speed.scalePercent <= speed.minScalePercent} onPress={() => sendTuple(commandPayloads.pointerSpeed(Math.max(speed.minScalePercent, speed.scalePercent - speed.stepPercent)))} /><ControlButton icon="add" label="Faster" disabled={!session.supports('pointer.speed.set') || !speed.setSupported || speed.scalePercent >= speed.maxScalePercent} onPress={() => sendTuple(commandPayloads.pointerSpeed(Math.min(speed.maxScalePercent, speed.scalePercent + speed.stepPercent)))} /></View></View> : null}
    {display?.supported && display.displayCount > 1 ? <View style={{ gap: spacing.sm }}><AppText accessibilityRole="header" variant="heading">Move to monitor</AppText><View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.sm }}>{(['left', 'up', 'down', 'right'] as const).map((direction) => <ControlButton key={direction} label={direction[0]!.toUpperCase() + direction.slice(1)} disabled={!session.supports('pointer.display.move')} onPress={() => sendTuple(commandPayloads.displayMove(direction))} />)}</View></View> : null}
  </View>;
