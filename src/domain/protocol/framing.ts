...
type Partial = { totalBytes: number; createdAt: number; chunks: Map<number, Uint8Array> };
...
export class FrameReassembler {
  ...
  accept(frame: BluetoothFrame): ReassemblyResult {
    ...
    this.clearExpired();
    const partial = this.#partials.get(frame.messageId) ?? { totalBytes: frame.totalBytes, createdAt: this.now(), chunks: new Map() };
    ...
    if (!frame.isFinal) return { kind: 'incomplete' };
