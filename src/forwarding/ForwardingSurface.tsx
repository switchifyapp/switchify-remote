...
let restoreDesktopId: string | null = null;
...
export function ForwardingSurface({ manager, bridge, profile, desktopId, preferences }: { manager: ConnectionManager; bridge: SwitchifyBridge; profile: PointerProfile; desktopId: string; preferences: Preferences }) {
  ...
  useEffect(() => {
    void controller.loadProfiles(preferences.forwardingProfiles[desktopId]).then(async () => { if (restoreDesktopId === desktopId) await controller.start(); });
    const unregister = manager.registerCleanup(async () => { restoreDesktopId = null; await controller.cleanup(); });
    return () => { unregister(); void controller.cleanup(); };
  }, [controller, desktopId, manager, preferences.forwardingProfiles]);
  ...
  return <View style={styles.root}>
    ...
    <ControlButton label={state.phase === 'active' ? 'Stop forwarding' : 'Start forwarding'} disabled={state.phase === 'starting' || state.profiles.length === 0} danger={state.phase === 'active'} onPress={() => {
      if (state.phase === 'active') { restoreDesktopId = null; void controller.stop(); }
      else void controller.start().then((started) => { if (started) restoreDesktopId = desktopId; });
    }} />
