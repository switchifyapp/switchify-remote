...
export type ForwardingMapping = ExternalSwitchInfo & { switchId: number; outputLabel: string | null; pressed: boolean };
...
export class ForwardingController {
  ...
  async start(): Promise<boolean> {
    ...
    const mappings = external.slice(0, 8).map((item, index) => ({ ...item, switchId: index + 1, outputLabel: selected.bindings.find((binding) => binding.switchId === index + 1 && binding.behavior !== 'unassigned')?.label ?? null, pressed: false }));
    if (!this.#legacy) {
  ...
  async stop(message: string | null = null): Promise<void> {
    ...
    this.#clearTimers();
    this.#set({ phase: 'idle', mappings: this.#state.mappings.map((mapping) => ({ ...mapping, pressed: false })), message });
  ...
  #accept(event: BridgeEvent): void {
    if (event.type === 'snapshot' && this.#state.phase === 'active' && (!event.captureAvailable || event.externalSwitches.length === 0)) { void this.stop('Switchify switch capture became unavailable.'); return; }
    if (event.type !== 'switchEdge' || event.generation !== this.#generation || this.#state.phase !== 'active') return;
    if (event.sequence !== this.#bridgeSequence + 1) { void this.stop('A switch event was missed. Forwarding stopped safely.'); return; }
    ...
    this.#set({ mappings: this.#state.mappings.map((item) => item.keyCode === event.keyCode ? { ...item, pressed: event.down } : item) });
    void this.#enqueue(async () => {
      await this.#edge(mapping.switchId, event.down);
      if (!event.down && !event.cancelled && duration >= this.holdToStopMs) void this.stop('Forwarding stopped after the switch was held.');
    });
  }
  ...
  #resetIdle(): void { if (this.#idle) this.timers.clear(this.#idle); this.#idle = this.timers.timeout(() => { void this.stop('Forwarding stopped after 60 seconds without switch activity.'); }, 60_000); }
