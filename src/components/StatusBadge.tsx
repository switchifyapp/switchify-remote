...
export function StatusBadge({ label, tone = 'neutral', icon }: { label: string; tone?: Tone; icon?: IconName }) {
  ...
  return <View accessible accessibilityLabel={label} style={{ alignItems: 'center', alignSelf: 'flex-start', backgroundColor: background, borderRadius: radii.pill, flexDirection: 'row', gap: spacing.xs, minHeight: 28, paddingHorizontal: spacing.md, paddingVertical: spacing.xs }}>
    <MaterialIcons color={foreground} importantForAccessibility="no" name={icon ?? 'circle'} size={icon ? 16 : 8} />
    <AppText importantForAccessibility="no" style={{ color: foreground }} variant="caption">{label}</AppText>
  </View>;
}
