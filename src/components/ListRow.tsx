...
export function ListRow({ title, description, icon, onPress }: { title: string; description?: string; icon?: IconName; onPress?: () => void }) {
  ...
  return <Pressable accessibilityRole={onPress ? 'button' : undefined} accessibilityLabel={onPress ? title : undefined} onPress={onPress} style={({ pressed }) => ({ alignItems: 'center', backgroundColor: pressed ? colors.surfacePressed : 'transparent', borderRadius: radii.md, flexDirection: 'row', gap: spacing.md, minHeight: 56, padding: spacing.md })}>
