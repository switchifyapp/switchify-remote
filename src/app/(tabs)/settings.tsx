...
export default function SettingsScreen() {
  ...
  return <Screen title="Settings" description="Preferences are stored only on this device.">
    <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: spacing.md }}>
      <View style={cardWidth}><SettingCard title="Opening surface" description="Shown first when the remote connects."><SelectorField label="Opening surface" options={surfaces.map((surface) => ({ key: surface, label: surface[0]!.toUpperCase() + surface.slice(1) }))} selectedKey={preferences.surface} onSelect={(surface) => void preferencesStore.update({ surface })} /></SettingCard></View>
