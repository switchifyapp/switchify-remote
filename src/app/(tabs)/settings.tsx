...
export default function SettingsScreen() {
  ...
  return (
    <Screen title="Settings" description="Preferences are stored only on this device.">
      <Text accessibilityRole="header" style={styles.heading}>Opening surface</Text><View style={styles.row}>{(['mouse', 'typing', 'window'] as const).map((surface) => <ControlButton key={surface} label={surface[0]!.toUpperCase() + surface.slice(1)} selected={preferences.surface === surface} onPress={() => void preferencesStore.update({ surface })} />)}</View>
