...
export default function PcsScreen() {
  ...
  return <Screen title="PCs" description="Connect securely to Switchify PC over Bluetooth.">
    {state.kind === 'pairing' ? <Card variant="hero" accessible accessibilityLabel={`Approve on your PC. Verification code ${state.verificationCode.split('').join(' ')}. Confirm that this code matches Switchify PC, then approve the request there.`} accessibilityLiveRegion="polite" accessibilityState={{ busy: true }}><StatusBadge icon="verified-user" label="Secure pairing" tone="brand" /><AppText importantForAccessibility="no" variant="title">Approve on your PC</AppText><AppText importantForAccessibility="no" variant="code">{state.verificationCode}</AppText><AppText importantForAccessibility="no" muted>Confirm that this code matches Switchify PC, then approve the request there.</AppText></Card> : null}
    ...
    {state.kind === 'connecting' || state.kind === 'reconnecting' ? <Card variant="hero" accessibilityLiveRegion="polite" accessibilityState={{ busy: true }}><StatusBadge icon="sync" label={state.kind === 'connecting' ? 'Connecting' : `Reconnect attempt ${state.attempt}`} tone="brand" /><AppText variant="title">{state.desktop.displayName}</AppText><AppText muted>{state.kind === 'connecting' ? `Connecting to ${state.desktop.displayName}…` : `Reconnecting to ${state.desktop.displayName}…`}</AppText></Card> : null}
    {state.kind === 'failed' ? <Card accessibilityRole="alert" variant="danger"><StatusBadge icon="error-outline" label="Connection failed" tone="danger" /><AppText variant="title">Could not connect</AppText><AppText muted>{state.message}</AppText></Card> : null}
